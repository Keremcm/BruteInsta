...
paths = {
    'guessPasswords': "wordlist.txt", # The path to gussed password file
...
def initialize():
    if module.uerExists(victim):
        if module.cacheFOUND(victim, paths['success']):
            with open(f"{paths['success']}{victim}.txt", "r") as file:
            ...
            try:
                UsedPasswords = open(
                    f"{paths['fail']}{victim}.txt", "r").read().split("\n")
